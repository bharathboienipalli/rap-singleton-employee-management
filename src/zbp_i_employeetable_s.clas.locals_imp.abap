CLASS LHC_EMPLOYEETABLEALL DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PUBLIC SECTION.
    CONSTANTS:
      CO_ENTITY TYPE abp_entity_name VALUE `ZI_EMPLOYEETABLE_SI`,
      CO_TRANSPORT_OBJECT TYPE mbc_cp_api=>indiv_transaction_obj_name VALUE `ZEMPLOYEETABLE`,
      CO_AUTHORIZATION_ENTITY TYPE abp_entity_name VALUE `ZI_EMPLOYEETABLE_BKR`.
  ...
  PRIVATE SECTION.
    METHODS:
      ...
      SELECTCUSTOMIZINGTRANSPTREQ FOR MODIFY
        IMPORTING
          KEYS FOR ACTION EmployeeTableAll~SelectCustomizingTransptReq
        RESULT result,
...
CLASS LSC_EMPLOYEETABLEALL IMPLEMENTATION.
  ...
  mbc_cp_api=>rap_bc_api( )->record_changes(
    ...
    entity           = lhc_EmployeeTableAll=>co_entity
    create           = REF #( create )
    update           = REF #( update )
    delete           = REF #( delete )
    reported         = REF #( reported ) ).
  mbc_cp_api=>rap_bc_api( )->update_last_changed_date_time(
    maintenance_object = 'ZEMPLOYEETABLE'
...
CLASS LHC_EMPLOYEETABLE DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PUBLIC SECTION.
    CONSTANTS:
      CO_ENTITY TYPE abp_entity_name VALUE `ZI_EMPLOYEETABLE_BKR`.
  ...
  PRIVATE SECTION.
    METHODS:
      ...
      VALIDATETRANSPORTREQUEST FOR VALIDATE ON SAVE
        IMPORTING
          KEYS_EMPLOYEETABLEALL FOR EmployeeTableAll~ValidateTransportRequest
          KEYS_EMPLOYEETABLE FOR EmployeeTable~ValidateTransportRequest.
ENDCLASS.
...
*    transport_object = lhc_EmployeeTableAll=>co_transport_object
*    entity           = lhc_EmployeeTableAll=>co_entity
*    validation_keys  = VALUE #( ( REF #( keys_EmployeeTableAll ) )
*                                ( REF #( keys_EmployeeTable ) ) )
*    failed           = REF #( failed )
*    reported         = REF #( reported ) ).
  ENDMETHOD.
ENDCLASS.
